...
hardeningAnalysis:
  epic3:
    ...
    userStories: 3
    hardeningStories: 2
    crossCuttingStories: 1
    totalStories: 6
    multiplier: 1.67
    patternsApplied: ['data-pipeline', 'error-resilience', 'input-sanitization', 'e2e-testing', 'pure-component']
    crossEpicFindings: 10
    additionalStoriesRecommended: 1
---
...
**Stories:** 3 user + 2 TD + 1 cross-cutting = 6 total
**Hardening Multiplier:** 1.67x (epic-scoped), 2.0x (with cross-cutting)
**Story Files:** `docs/sprint-artifacts/stories/epic3/`

### TD-CROSS-1: Shared Test Infrastructure (Factories, Fixtures, Mock Patterns)

Cross-cutting tech debt. Extracts duplicated test factory functions (makeComponent, makeMetric, makeNode, makeEdge) from 8+ test files and inconsistent componentLibrary mock patterns from 6+ files into shared helpers. Recommended to run BEFORE Story 3-1 to prevent compounding duplication.

**Sizing:** SMALL (2 tasks, ~10 subtasks, ~6 files)
**Status:** ready-for-dev
**File:** `docs/sprint-artifacts/stories/epic3/td-cross-1-shared-test-infrastructure.md`
...
This story creates the full YAML import pipeline: the architecture file Zod schema (with versioning), the import service (parse -> validate -> sanitize -> hydrate -> recalculate), the import UI (toolbar button + file picker + validation error display), and placeholder nodes for unknown components. It is the security boundary where all untrusted user data enters the application.

**FRs:** FR23, FR25, FR27
**NFRs:** NFR4, NFR6, NFR7, NFR8, NFR9, NFR10
**ARs:** AR9, AR10
**Risk:** HIGH (primary attack surface)
**Sizing:** LARGE (7 tasks, ~38 subtasks, ~11 files)
**Status:** ready-for-dev
**File:** `docs/sprint-artifacts/stories/epic3/story-3-1.md`
**Hardening BUILT-IN:** Pattern 1 (schema .strict(), validation), Pattern 2 (ErrorBoundary), Pattern 3 (sanitizeDisplayString, URL validation), Pattern 4 (data-testid, E2E)

**Key Implementation Note:** loadArchitecture() MUST use `recalculateArchitecture()` bulk path (not per-node `triggerRecalculation()`) — avoids O(n^2) computation and overlapping ripple animations on import. Must also clear all computed state including in-flight ripple timeouts.

### TD-3-1a: Import Pipeline Robustness

Tech debt from Story 3-1. Adversarial test coverage for the YAML import pipeline: deeply nested YAML, massive arrays, Unicode homoglyphs, concurrent import guard, schema version edge cases.

**Sizing:** SMALL (2 tasks, ~14 subtasks, ~3 files)
**Status:** ready-for-dev
**File:** `docs/sprint-artifacts/stories/epic3/td-3-1a-import-pipeline-robustness.md`
...
This story creates the YAML export service (dehydrate canvas state -> skeleton YAML), the export button in the toolbar, and validates lossless round-trip fidelity.
...
**Status:** ready-for-dev
**File:** `docs/sprint-artifacts/stories/epic3/story-3-2.md`
**Hardening BUILT-IN:** Pattern 1 (schema validation on export), Pattern 4 (E2E), Blob URL cleanup
...
This story creates 2 example blueprints (WhatsApp, Telegram), populates the Blueprints tab, implements blueprint loading (reusing import pipeline), and creates a static AI prompt template.

**FRs:** FR28, FR29, FR30
**Risk:** MEDIUM
**Sizing:** LARGE (7 tasks, ~32 subtasks, ~10 files)
**Status:** ready-for-dev
**File:** `docs/sprint-artifacts/stories/epic3/story-3-3.md`
**Hardening BUILT-IN:** Pattern 1 (seed validation, batch chunking), Pattern 2 (ErrorBoundary), Pattern 4 (E2E), Pattern 5 (empty state)

### TD-3-3a: Seed & Blueprint Hardening

Tech debt from Story 3-3. Seed script cross-reference validation, blueprint cache miss resilience, empty skeleton handling, seed idempotency testing.

**Sizing:** SMALL (2 tasks, ~13 subtasks, ~4 files)
**Status:** ready-for-dev
**File:** `docs/sprint-artifacts/stories/epic3/td-3-3a-seed-blueprint-hardening.md`

### Cross-Epic Analysis Findings

**HIGH:** Story 3-1 must use `recalculateArchitecture()` bulk path for import — documented in story dev notes.
**MEDIUM:** Test factory duplication across 8+ files — addressed by TD-CROSS-1.
**MEDIUM:** componentLibrary mock inconsistency — addressed by TD-CROSS-1.
**MEDIUM:** Epic 4 needs 2 new Firestore collections (AR12, AR13) — noted for Epic 4 planning.
**LOW:** BlueprintSchema additive extension (no migration needed), bare console.warn in 2 Epic 2 files, architectureStore size tracking, sanitize coverage for architecture name, connectionProperties vs connectionTypes overlap.
