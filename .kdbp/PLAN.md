...
| 4 | Author the hard challenges | Recast all 41 IN PLACE (same ids, D67) onto typed trafficSources + harden with Phase 3 levers; 1-5 progressive hint ladders (last=full solution); solvability harness (reference solution per challenge must clear). Sub-slices 4a harness → 4b recast → 4c harden → 4d hints. | ent | high | ✅ | ⬜ | ✅ | ⬜ |
...
Phase 4: Author the hard challenges — Phase 3 COMPLETE (3a/3c/3f/3b/3d/3e shipped+local). Strategy locked (D67): harden in-place + solvability harness + progressive hints. Next: build 4a (solvability harness) on the real sim+scorer primitives.
