...
## D98 — Plan-2 Phase 3 design lock: coach de-escalation, beginner-gated (2026-06-11)
...
## D101 — Break economy v2: precision-gated rps + causal categorical breaks; achieved-star standing (2026-06-11)

**Owner forks (playtest):** (1) precision ONLY, no dial cap — the rps dial stays free, but the expert pays only when the breaking value is ≤2× the build's TRUE failure boundary (engine-probed binary search, ≤12 sims, mirrors the live canvas-wins seam); overshoot = "broke it, paid nothing, find the edge". (2) The owner's combination semantics for Shape/Workload/Origin: rps may co-deviate; the categorical pays iff CAUSAL — a counterfactual probe with the authored value at the same load must PASS while the deviated run failed ("it would have broken anyway" pays nothing). Collected rps breaks REVEAL the boundary (earned knowledge).
**Probe parity:** src/services/breakProbe.ts mirrors simulationLaunch's post-3★ path (node-built curve, canvas multiRegion, authored events/duration/chaos); pass predicate = rubricScorer passedMetrics minus cost-per-request. KEY ENGINE FACT: the live path peaks at the DIAL value (node-built curve), so first-service's live boundary ≈1055 rps (the lean compute's saturation), NOT the ~154 a spec-curve probe suggests. Drift rule: probe can't reproduce the failure ⇒ collect (never punish on drift). V1 scope: single-source quests; multi-source keep legacy single-attribute detection.
