        ...
        if "e2e" in file_path and file_path.endswith(".spec.ts"):
            if line_count > 400:
                warnings.append(
                    ...
                    "Consider splitting into separate journeys."
                )

    # 7. E2E anti-pattern detection (only for e2e spec files)
    if "e2e" in file_path and file_path.endswith(".spec.ts"):
        # Long fixed timeouts for async operations
