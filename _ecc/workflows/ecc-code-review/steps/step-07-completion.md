...
<step n="7" goal="Cost tracking, E2E analysis, and story status update" tag="completion">
  ...
  <output>**ECC Code Review Complete!**
    ...
    **Commit Commands:**
    ...
    {{/each}}
    git commit -m "Review {{story_key}}: {{overall_status}} {{overall_score}}/10{{#if td_story_count}}, create {{td_stories_list}}{{/if}}" -m "Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>"
    ```
