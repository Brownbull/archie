...
| `feedback-phase4/` | `feedback-phase4.unlocked.spec.ts` | P4 (D94) | Phase-4 break-it loop journey: production-ai 3★ + "Now break it" invite → web-users RPS dial 2k→12k → re-run fails → break collected (+1 Expert, Peak RPS) → reset dials restores authored spec → workload write-heavy → second break (Workload) |
| `feedback-phase5/` | `feedback-phase5.unlocked.spec.ts` | P5 (D95) | Phase-5 runtime evidence: chain panel (Stream the Data, stage 2 of 3 + fork targets), team-expertise restriction line (Scale Out), named chaos objective (Observe to Recover), brownfield seeding (accepting Scale Out lands the inherited app box WIRED to traffic), and the default-view Link Legend (protocol line styles + dot-speed cue) |
| `plan2-phase2/` | `plan2-phase2.unlocked.spec.ts` | Plan-2 P2 (D97) | Chain & extras legibility: stage badges + chain-blue edges (startup-stack fork at first-service), resilience shields pre-completion, widened extras panel + edge-line chain info |
