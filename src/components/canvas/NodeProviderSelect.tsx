  ...
  if (providers.length <= 1) {
    return (
      <div data-testid="archie-node-variant" className="flex items-center gap-1 px-3 pb-0.5">
        ...
        <span className="truncate text-[0.625rem] text-text-secondary">
    ...
    >
      <Select value={componentId} onValueChange={(v) => swapNodeComponent(nodeId, v)}>
        <SelectTrigger
          ...
          className="h-6 w-full gap-1 border-archie-border bg-surface px-1.5 py-0 text-[0.625rem] text-text-secondary"
        >
        ...
        <SelectContent>
          {providers.map((p) => {
            ...
            return (
              <SelectItem key={p.id} value={p.id} className="text-[0.6875rem]">
